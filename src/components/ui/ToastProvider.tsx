...
const HeroToast = ({
  ...
  return (
    <Card
      className={`w-80 shadow-lg border-l-4 ${type === "success" ? "border-success" : type === "error" ? "border-danger" : "border-primary"}`}
    >
      <CardBody className="flex flex-row items-center gap-3 p-3 overflow-hidden">
        {/* Icono */}
        <div
          className={`p-2 rounded-full ${style.color} ${style.text} text-lg`}
        >
          {style.icon}
        </div>
